...
router.put("/:name", async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    // changed, or a text command was just promoted to a prompt (in which case the
    // stored text becomes prompt input and has never been screened).
    //
    // Metadata-only edits on an existing prompt — toggling enabled, changing a
    // cooldown — deliberately do not re-screen. The text is unchanged, so a repeat
